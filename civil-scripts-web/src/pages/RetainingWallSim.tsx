    ...
    <div className="relative h-[calc(100vh-8rem)] w-full overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--color-border)] bg-[var(--color-background)] shadow-lg flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col relative p-6 overflow-y-auto space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricDisplay label="SF Guling (Keamanan)" value={result.SF_overturning} unit="Ratio" status={result.SF_overturning >= 1.5 ? "safe" : "danger"} />
          <MetricDisplay label="SF Geser (Sliding)" value={result.SF_sliding} unit="Ratio" status={result.SF_sliding >= 1.5 ? "safe" : "danger"} />
          <MetricDisplay label="Tegangan Tanah (q_max)" value={result.q_max} unit="kPa" status={result.q_max > (soilType === "pasir" ? 200 : 100) ? "danger" : "safe"} />
          <MetricDisplay label="Eksentrisitas (e / (B/6))" value={result.e_ratio} unit="Ratio" status={result.e_ratio > 1 ? "warning" : "safe"} />
        </div>
